...
<div class="detail">
    ...
    <div class="c_crumbs"><div><b></b><strong>我的预约</strong></div></div>
  <table id="baseInfo" width="100%" align="center" class="list" border="0" cellpadding="0" cellspacing="0"  >
        ...
        <tr>
            ...
            <td>${mySubInfo.matter }</td>
        </tr>
        <tr>
            <td class="tdBg">预约事项名称：</td>
            <td>${mySubInfo.matter_name }</td>
        </tr>
        
		<tr>
            <td class="tdBg">预约时间：</td>
            <td> ${mySubInfo.sub_time}           
            </td>
        </tr>
		<tr>
            <td class="tdBg">预约地点：</td>
            <td>    ${mySubInfo.sub_place }        
            </td>
        </tr>
		<tr>
            <td class="tdBg">预约说明：</td>
            <td>  ${mySubInfo.sub_desc }        
            </td>
        </tr>
	...
	<tr>
            ...
            <td>
            <c:choose>
             <c:when test="${mySubInfo.sub_state  == '0' }">预约成功</c:when>
             <c:when test="${mySubInfo.sub_state  == '1' }">预约失败</c:when>
             <c:when test="${mySubInfo.sub_state  == '2' }">处理中</c:when>
        ...
        <tr>
            ...
            <td>
            <c:choose>
             <c:when test="${mySubInfo.reply_content != null }">${mySubInfo.reply_content}</c:when>
            <c:otherwise>处理中，请耐心等待……</c:otherwise>
            </c:choose>
